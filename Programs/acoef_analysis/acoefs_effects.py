...
from acoefs import *
from activity import a_model_cpp
...
def a2_epsilon_plot(lrange=[1,2], colors=['Blue', 'Orange', 'Red'], ftype='gate', fileout=None):
    '''
    ...
    for l in lrange:
        ...
        ax.plot(epsilon_nl[1:], a2_AR_eq, color=colors[j], linestyle='--')
        ax.plot(epsilon_nl[1:], a2_AR_polar, color=colors[j], linestyle='dashdot')
        ax.plot(epsilon_nl, np.repeat(a2_CF,len(epsilon_nl)), linestyle='solid', color=colors[j], label='l = ' + str(l))
        j=j+1
    #
    ax.axvline(x=5e-4, linestyle='--', color='black', label='Solar Value') # Show the solar value
    ax.set_title('a2(CF) + a2(AR) coefficient in function of epsilon, a1 =' + str(a1) + 'nHz')
    ax.set_ylabel('a2 (nHz)')
