...
def bin2txt(dir_tamcmc_outputs, process_name, phase='A', chain=0, first_index=0, last_index=-1, period=1, erase_tmp=True):
	'''
	...
	process=run(["cpp_prg/./bin2txt", core_filename, str(chain), outdir, str(first_index), str(last_index), str(period)])
	#time.sleep(1.)
	#
	...
	if len(files) != 0:
		...
		print('           cpp_prg/./bin2txt ', core_filename, ' ', str(chain), ' ', outdir, ' ', str(first_index), ' ', str(last_index), ' ', str(period))
		print(' --- ')	
