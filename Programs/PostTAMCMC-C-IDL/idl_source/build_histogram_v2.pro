...
pro show_hist_aj_inc_matrix, a1, inc, a2=a2, a3=a3, a4=a4, a5=a5, a6=a6, extra=extra, dir_out, name_out
  ...
  if n_elements(a2) ne 0 then if total(a2) ne 0 then  dim=dim+1
  if n_elements(a3) ne 0 then if total(a3) ne 0 then  dim=dim+1
  if n_elements(a4) ne 0 then if total(a4) ne 0 then dim=dim+1
  if n_elements(a5) ne 0 then if total(a5) ne 0 then  dim=dim+1
  if n_elements(a6) ne 0 then if total(a6) ne 0 then  dim=dim+1
  ...
  ;
  j=1
  st=set_names(a2, 'a2', j)
  if st.data[0] ne 0 then data[j,*]=st.data*1000
  if st.data[0] ne 0 then names[j]=st.name
  j=st.ind
  st=set_names(a3, 'a3', j)
  if st.data[0] ne 0 then data[j,*]=st.data*1000
  if st.data[0] ne 0 then names[j]=st.name
  j=st.ind
  st=set_names(a4, 'a4', j)
  if st.data[0] ne 0 then data[j,*]=st.data*1000
  if st.data[0] ne 0 then names[j]=st.name
  j=st.ind
  st=set_names(a5, 'a5', j)
  if st.data[0] ne 0 then data[j,*]=st.data*1000
  if st.data[0] ne 0 then names[j]=st.name
  j=st.ind
  st=set_names(a6, 'a6', j)
  if st.data[0] ne 0 then data[j,*]=st.data*1000
  if st.data[0] ne 0 then names[j]=st.name
  j=st.ind
  ;stop
  ...
  for i=0, n_elements(data[*,0])-1 do begin
    ...
    if names[i] eq "a2" then begin
      labels[i]=textoidl('a_2' + ' (nHz)')
      label_extra=labels[i]
    endif
    if names[i] eq "a3" then begin
      labels[i]=textoidl('a_3' + ' (nHz)')
    endif
    if names[i] eq "a4" then begin
      labels[i]=textoidl('a_4' + ' (nHz)')
    endif
    if names[i] eq "a5" then begin
      labels[i]=textoidl('a_5' + ' (nHz)')
    endif
    if names[i] eq "a6" then begin
      labels[i]=textoidl('a_6' + ' (nHz)')
    endif
endfor
...
function set_names, aj,  name, ind
  if n_elements(aj) ne 0 then begin
    if total(aj) ne 0 then begin
     data=aj
     ind=ind+1
    endif else begin
      data=dblarr(n_elements(aj))
    endelse
  endif
  struc={name:"", data:dblarr(n_elements(aj)), ind:0}
  struc.name=name
  struc.data=data
  struc.ind=ind
  return, struc
