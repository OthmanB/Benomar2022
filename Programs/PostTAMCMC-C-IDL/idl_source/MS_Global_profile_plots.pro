...
posOK=where(Stat_Synthese_freq[3,0,*] ne 0)
Nmax=n_elements(posOK)
;Nmax=n_elements(stat_synthese_Height[0,0,*])

mini=1e6
for el=0, n_elements(Stat_Synthese_freq[3,*,0])-1 do begin
	pos=where(Stat_Synthese_freq[3,el,*] gt 0)
	if mini > min(Stat_Synthese_freq[3,el,pos]) then mini = min(Stat_Synthese_freq[3,el,pos]) 
endfor
maxi=max(Stat_Synthese_freq[3,*,*])
...
borne_sup=max(stat_Synthese_Height[4,0,posOK]+Bruit_local[0,posOK])*1.2; permet de tronquer la barre d'erreur à 2 sigma si elle depasse de la zone de graphique (3.5 pour HD141820)
print, 'Upper bound for graphs of the Heights: '+strtrim(borne_sup,1)
plotsym, 0, 1., /fill
plot, Stat_Synthese_freq[3,0,posOK],/NoData,$;/ylog,$
	psym=8,background=255,color=1,yr=[0.0,borne_sup],$
	xtitle='Frequency ('+frequency_unit+')',ytitle='Height ('+height_unit+')',$
	xr=[0.9*mini,1.05*maxi],charsize=2.,/yst,/xst
