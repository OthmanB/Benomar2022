...
;stop
asymp=check_fix(asymp, 0)
a2p=check_fix(a2p, 1d-10)
a3p=check_fix(a3p, 0)
mag_bp=check_fix(mag_bp, 0)
mag_alfap=check_fix(mag_alfap, 0)
...
pro MS_Global_rotinc_correlations_aj, file_synthese, dir_files_samples, dir_out, identifier
...
Nf=total(Nf_ls) ; The total number of parameter is total(Nf_ls)
;k=parameters_length[3]
;l=parameters_length[4]
;mm=parameters_length[5]
...
restore, dir_files_samples  + format_filename(i_a1, idl_format) +'.sav'
if n_elements(pos_OK) eq 0 then pos_OK=where(param ge 0) ; Basically all values by default
a1p=param[pos_OK] ; WE IGNORE a1_1 here ... (no slope with freq)

restore, dir_files_samples  + format_filename(i_a1 + 2, idl_format) +'.sav'
a2p=param[pos_OK]

restore, dir_files_samples + format_filename(i_a1 + 4, idl_format) +'.sav'
a3p=param

restore, dir_files_samples + format_filename(i_a1 + 6, idl_format) +'.sav'
a4p=param[pos_OK]

restore, dir_files_samples + format_filename(i_a1 + 8, idl_format) +'.sav'
a5p=param[pos_OK]

restore, dir_files_samples + format_filename(i_a1 + 10, idl_format) +'.sav'
a6p=param[pos_OK]

restore, dir_files_samples + format_filename(i_a1 + 12, idl_format) +'.sav'
eta0=param[pos_OK]

restore, dir_files_samples + format_filename(i_a1 + 13, idl_format) +'.sav'
asymp=param[pos_OK]

restore, dir_files_samples  + format_filename(ind0_inc, idl_format) +'.sav'
incp=param[pos_OK]
...
;stop
if (stddev(a1p) ne 0 AND stddev(incp) ne 0) AND n_elements(a1p) gt 1 AND n_elements(incp) gt 1 then begin 
	show_hist_aj_inc_matrix, a1p, incp, a2=a2p, a3=a3p, a4=a4p, a5=a5p, a6=a6p, extra=0, dir_out, name_out + 'aj_'
	show_hist_aj_inc_matrix, a1p, incp, a2=0, a3=0, a4=0, a5=0, a6=0, extra=0, dir_out, name_out+'a1inconly_'  ; all except asymetry
endif else print, 'No correlation diagram to show: a1 and/or inclination are fixed'
...
function check_fix, param, thld
	p=param
	if n_elements(p) gt 1 then begin 
		if variance(p) le thld then p=0
	endif else begin
		p=0
	endelse
return, p
